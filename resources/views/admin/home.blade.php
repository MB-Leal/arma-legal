@extends('admin.template')

@section('content')
    <x-navbar></x-navbar>

    <main class="flex justify-around items-center flex-col overflow-x-hidden w-[100vw] min-h-[90vh] ">

        <header class="text-center text-white ">
            <h1 class="text-6xl">Painel Administrativo</h1>
            @if (session('success'))
                <span>{{ session('danger') }}</span>
            @endif
        </header>
        <div class="w-[80%] h-[20em] mb-4 flex flex-col lg:flex-row gap-4 items-center ">
            <div
                class="lg:w-[50%] w-[100%] h-[100%] border flex justify-center items-center bg-green-600 hover:bg-green-700">

                <a href="{{ route('admin.associado') }}"
                    class="w-[100%] h-[100%] justify-center items-center flex text-white font-bold text-2xl">
                    <p class="mr-2">Gerenciar Associados</p>
                    <svg height="50px" width="50px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
                        xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 512 512" xml:space="preserve">
                        <path style="fill:#FFFFFF;"
                            d="M256,508C117.04,508,4,394.96,4,256S117.04,4,256,4s252,113.056,252,252S394.96,508,256,508z" />
                        <g>
                            <path style="fill:#D6D6D6;"
                                d="M256,8c136.752,0,248,111.248,248,248S392.752,504,256,504S8,392.752,8,256S119.248,8,256,8 M256,0
    C114.608,0,0,114.608,0,256s114.608,256,256,256s256-114.608,256-256S397.392,0,256,0L256,0z" />
                            <circle style="fill:#D6D6D6;" cx="256" cy="103.072" r="32" />
                        </g>
                        <g>
                            <path style="fill:#333333;"
                                d="M254.48,209.552l-30.944-67.088c0,0-32.464,0.432-32.464,30.304s0,36.784,0,36.784
    S254.704,209.552,254.48,209.552z" />
                            <path style="fill:#333333;"
                                d="M257.52,209.552l30.944-67.088c0,0,32.464,0.432,32.464,30.304s0,36.784,0,36.784
    S257.296,209.552,257.52,209.552z" />
                        </g>
                        <polygon style="fill:#0BA4E0;" points="256,142.464 233.712,142.464 256,191.808 278.288,142.464 " />
                        <circle style="fill:#D6D6D6;" cx="165.36" cy="267.856" r="32" />
                        <g>
                            <path style="fill:#333333;"
                                d="M163.856,374.336l-30.944-67.088c0,0-32.464,0.432-32.464,30.304s0,36.784,0,36.784
    S164.064,374.336,163.856,374.336z" />
                            <path style="fill:#333333;"
                                d="M166.88,374.336l30.944-67.088c0,0,32.464,0.432,32.464,30.304s0,36.784,0,36.784
    S166.672,374.336,166.88,374.336z" />
                        </g>
                        <polygon style="fill:#0BA4E0;"
                            points="165.36,307.248 143.072,307.248 165.36,356.592 187.664,307.248 " />
                        <circle style="fill:#D6D6D6;" cx="346.64" cy="267.856" r="32" />
                        <g>
                            <path style="fill:#333333;"
                                d="M345.12,374.336l-30.944-67.088c0,0-32.464,0.432-32.464,30.304s0,36.784,0,36.784
    S345.328,374.336,345.12,374.336z" />
                            <path style="fill:#333333;"
                                d="M348.144,374.336l30.944-67.088c0,0,32.464,0.432,32.464,30.304s0,36.784,0,36.784
    S347.936,374.336,348.144,374.336z" />
                        </g>
                        <polygon style="fill:#0BA4E0;"
                            points="346.64,307.248 324.336,307.248 346.64,356.592 368.928,307.248 " />
                    </svg>
                </a>
            </div>
            <div class="border lg:w-[50%] w-[100%] h-[100%] flex justify-center items-center bg-blue-700 hover:bg-blue-800">
                <a href="{{ route('admin.arma') }}"
                    class="w-[100%] h-[100%] justify-center items-center flex text-white font-bold text-2xl">
                    <p class="mr-2">Gerenciar Armas</p>
                    <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
                        xmlns:xlink="http://www.w3.org/1999/xlink" style="width: 50px;" viewBox="0 0 496.158 496.158"
                        xml:space="preserve">
                        <path style="fill:#337180;"
                            d="M248.082,0.003C111.071,0.003,0,111.062,0,248.085c0,137.001,111.07,248.07,248.082,248.07
	c137.006,0,248.076-111.069,248.076-248.07C496.159,111.062,385.088,0.003,248.082,0.003z" />
                        <g>
                            <path style="fill:#A6A7A9;"
                                d="M103.57,150.833c-5.877-1.958-7.835-6.53-9.14-1.305c-1.305,5.224,21.547,18.283,21.547,18.283
		l6.53-12.406C122.507,155.405,109.448,152.792,103.57,150.833z" />
                            <path style="fill:#A6A7A9;"
                                d="M262.067,179.563h-72.48c-8.653,0-17.55,7.017-19.87,15.672l-10.842,40.485
		c-2.318,8.654,2.819,15.67,11.473,15.67h65.297c16.325,0,24.954-7.836,29.502-24.813l8.395-31.343
		C275.86,186.58,270.722,179.563,262.067,179.563z M261.611,211.742l-4.806,17.939c-2.602,9.716-7.541,14.201-16.883,14.201h-30.997
		c-4.954,0-7.892-4.017-6.564-8.969l6.206-23.171c1.326-4.953,6.417-8.968,11.37-8.968h35.105
		C259.997,202.773,262.937,206.789,261.611,211.742z" />
                            <path style="fill:#A6A7A9;"
                                d="M419.935,145.609H129.69l-11.038,7.854l-11.816,48.955h313.1c0,0,4.57-10.448,4.57-28.078
		C424.505,156.71,419.935,145.609,419.935,145.609z" />
                        </g>
                        <g>
                            <path style="fill:#BCBCBC;"
                                d="M421.567,145.609c0,0-284.694,0-291.877,0s-8.925,2.395-10.653,6.479
		c-1.878,4.438-7.56,37.091-11.114,39.882c-3.554,2.793-19.554,2.363-17.412,7.182c2.414,5.43,29.871,6.976,28.906,29.82
		c-2.698,7.436-5.972,15.014-10.623,23.072c-7.113,12.331-20.896,26.119-31.996,50.279c-11.101,24.16-5.225,43.749-5.225,43.749
		s12.888,3.361,33.734,4.354c20.846,0.992,50.5-4.354,50.5-4.354s54.087-128.412,57.08-133.748s8.217-7.947,15.4-7.947
		c7.182,0,121.452,0,121.452,0s3.83-1.256,3.809-1.959c0,0-3.868,0.227-3.87-0.165c-0.053-5.53,0.532-20.421,7.896-27.26
		c9.142-8.489,68.563-5.877,68.563-5.877C426.138,161.608,421.567,145.609,421.567,145.609z" />
                            <path style="fill:#BCBCBC;"
                                d="M349.716,202.31c-0.025,0.761-0.037,1.453-0.039,2.067c0.033,0,0.063,0,0.063,0
		s3.83-1.256,3.809-1.959C353.549,202.418,350.09,202.62,349.716,202.31z" />
                        </g>
                        <path style="fill:#888889;"
                            d="M425.963,169.109c-3.526-0.146-59.534-2.336-68.388,5.885c-7.365,6.839-7.949,21.73-7.896,27.26
	c0,0.021,0.016,0.04,0.037,0.057c0.215-6.723,1.54-18.51,7.859-24.379c9.142-8.489,68.563-5.876,68.563-5.876
	C426.138,171.178,426.074,170.18,425.963,169.109z" />
                        <path style="fill:#8B8B8C;"
                            d="M193.581,192.051c0,0-20.469,0.241-27.886,0.241c-7.416,0-14.812,1.44-18.134,9.433l-69.63,145.682
	c5.765,1.054,15.212,2.439,27.378,3.019c11.801,0.562,26.421-0.908,36.891-2.289l61.275-146.7
	C206.797,193.446,200.536,192.051,193.581,192.051z" />
                        <path style="fill:#A6A7A9;"
                            d="M112.167,181.523l-0.609,2.611H352.17c0.298-0.889,0.621-1.766,0.991-2.611H112.167z" />
                        <rect x="158.42" y="148.439" style="fill:#9C9DA0;" width="48.32" height="33.08" />
                        <path style="fill:#C6C6C6;"
                            d="M421.567,145.609c0,0-284.694,0-291.877,0c-6.822,0-8.734,2.162-10.391,5.876h303.82
	C422.267,148.058,421.567,145.609,421.567,145.609z" />
                        <g>
                            <path style="fill:#9C9DA0;"
                                d="M113.322,318.145c-2.543,3.572-2.223,8.553,0.98,11.754c3.203,3.203,8.182,3.523,11.754,0.981
		L113.322,318.145z" />
                            <path style="fill:#9C9DA0;"
                                d="M127.23,316.972c-3.257-3.256-8.35-3.532-11.932-0.85l12.782,12.781
		C130.763,325.322,130.484,320.228,127.23,316.972z" />
                        </g>
                    </svg>
                </a>
            </div>
            <div
                class="border lg:w-[50%] w-[100%] h-[100%] flex justify-center items-center bg-purple-700 hover:bg-purple-800">
                <a href="{{ route('admin.usuario') }}"
                    class="w-[100%] h-[100%] justify-center items-center flex text-white font-bold text-2xl">
                    <p class="mr-2">Adicionar Usuário</p>
                    <svg width="50px" height="50px" viewBox="0 0 48 48" xmlns="http://www.w3.org/2000/svg">
                        <title>70 Basic icons by Xicons.co</title>
                        <rect x="3" y="3" width="42" height="42" rx="4" ry="4"
                            fill="#d7effa" />
                        <path d="M24,34.25a2,2,0,0,1-2-2V15.75a2,2,0,0,1,4,0v16.5A2,2,0,0,1,24,34.25Z" fill="#38b1e7" />
                        <path d="M32,26H16a2,2,0,0,1,0-4H32A2,2,0,0,1,32,26Z" fill="#38b1e7" />
                    </svg>
                </a>
            </div>
        </div>

    </main>

    <x-footer></x-footer>


@endsection
