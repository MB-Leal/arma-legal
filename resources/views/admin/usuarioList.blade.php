...
@include('components.buttonReturn')
    ...
    <main class="flex flex-col w-[100vw] items-center h-[98vh]">
        ...
        @endif
        @if (session('error'))
        <div id="alert-border-3" class="flex p-4 mb-4 text-red-800 mt-4 w-[90%] m-auto border-t-4 border-blue-300 bg-blue-50 dark:text-blue-400 dark:bg-gray-800 dark:border-blue-800" role="alert">
            <svg class="flex-shrink-0 w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"></path></svg>
            <div class="ml-3 text-sm font-medium">
                <p>{{session('error')}}</p>
            </div>
                    ...
                    <tbody>
                        @foreach ($resultado as $res)
                            <tr class="bg-white border-b   hover:bg-gray-50 ">
                                ...
                                <td class="px-6 py-4 border">
                                    {{ \Carbon\Carbon::parse($res->updated_at)->format('d/m/Y')}}
                                </td>
                                <td class="px-6 py-4 border text-center flex flex-col items-center justify-center gap-2">
                                    ...
                                    <form class="d-inline" action="{{route('admin.deletarUser',['id'=>$res->id])}}" method="post" onsubmit="return confirm('Tem certeza que deseja excluir este usuário?')">
                                        @method('DELETE')
                                        @csrf
        ...
        <!-- Modal do botão de adicionar novo associado-->
        <div class="w-[50%] border rounded-md text-center bg-purple-700 hover:bg-purple-600 mb-4">
            <button data-modal-target="staticModal" data-modal-toggle="staticModal"
                class="p-4 text-white text-center w-full" type="button">
        ...
        <div id="staticModal" data-modal-backdrop="static" tabindex="-1" aria-hidden="true"
            ...
            <div class="relative w-[60%] max-h-full">
                ...
                <div class="relative flex flex-col items-center bg-white rounded-lg shadow ">
                    ...
                    <!-- Modal body -->
                    <form method="post" action="{{ route('admin.cadUser') }}" class="w-[100%] p-2 flex flex-col justify-around items-center">
                    @csrf
